...
public class LogAspect {
    ...
    private static final ThreadLocal<String> LOG_PREFIX_THREAD_LOCAL = new NamedThreadLocal<>("ThreadLocal LogPrefix");
     ...
     * @param
     */
    @Pointcut("execution(public * com.yhlo..*.*(..))")
    public void excute() {
    }
    ...
    public void doBefore(JoinPoint joinPoint) {
        ...
        for (int i = 0; i < args.length; i++) {
            argsDes.append(args[i]);
        }

        String logPrefix = className + "." + methodName;
        LOG_PREFIX_THREAD_LOCAL.set(logPrefix);
        log.info("{} 入参为:{}", logPrefix, JSONObject.toJSON(argsDes.toString()));
    }
